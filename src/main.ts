...
function analyticsControlsView() {
...
function bindAnalyticsControls(host: HTMLElement) {
...
async function renderRecordAnalytics() {
  ...
  if (!host) return;
  const { analyzeRecords, trendPolyline, selectAnalyticsRecords } = await import("./recordAnalytics");
  if (!host.isConnected) return;
  const limit = analyticsRange === "all" ? null : Number(analyticsRange);
  const records = selectAnalyticsRecords(filteredPracticeRecords(), {
    limit,
    dateFrom: analyticsDateFrom,
    dateTo: analyticsDateTo,
  });
  if (!records.length) {
    host.innerHTML = `<header><div><p class="eyebrow">PROGRESS</p><h2>練習の進み方</h2></div><span>0回を分析</span></header>
      ${analyticsControlsView()}
      <p class="analytics-empty">指定した範囲に記録がありません。</p>`;
    bindAnalyticsControls(host);
    return;
  }
